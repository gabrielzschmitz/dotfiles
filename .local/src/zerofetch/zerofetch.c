...
static char *get_title() {
    ...
    snprintf(title, BUF_SIZE, COLORT"%s@"COLORT"%s ~", username, hostname);
...
static char *get_battery_percentage() {
  ...
  snprintf(battery, 20, "%d%%", battery_capacity);
...
char *get_cache_file() {
    ...
    if(env == NULL)
        snprintf(cache_file, BUF_SIZE, "%s/.cache/zerofetch", getenv("HOME"));
    else
        snprintf(cache_file, BUF_SIZE, "%s/zerofetch", env);
