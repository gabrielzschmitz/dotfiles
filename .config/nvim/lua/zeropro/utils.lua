...
function utils.warn(...)
    for _, msg in ipairs({ ... }) do
        vim.cmd('echohl WarningMsg | echom "zeropro.nvim: ' .. msg .. '" | echohl NONE')
    end
end
...
function utils.load_theme(theme)
    ...
    if vim.g.colors_name == "zeropro" and vim.g.zeropro_style == theme.colors.name then
        return
    end
    ...
    if vim.g.colors_name == nil then
        vim.g.zeropro_config = theme.config
    end
    local config = vim.g.zeropro_config or theme.config

    vim.o.termguicolors = true
    vim.g.colors_name = "zeropro"
    vim.g.zeropro_colors = theme.colors
    vim.g.zeropro_style = theme.colors.name
